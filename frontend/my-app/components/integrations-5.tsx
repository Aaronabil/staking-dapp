import { Gemini, MetaMask, Walletconnect, Trustwallet, Rabbywallet, Brave } from '@/components/logos'
                ...
                <div className="mx-auto max-w-5xl px-6">
                    <div className="aspect-16/10 group relative mx-auto flex max-w-[22rem] items-center justify-between sm:max-w-sm">
                        ...
                        <div className="absolute inset-x-0 bottom-0 mx-auto my-2 flex w-fit justify-center gap-2">
                            <div className="bg-muted relative rounded-full border p-1">
                                <IntegrationCard
                    ...
                    <div className="bg-linear-to-t from-background relative mx-auto mt-12 max-w-lg space-y-6 from-55% text-center">
                        <h2 className="text-balance text-3xl font-semibold md:text-4xl">Connect With Your Favorite Wallet</h2>
                        <p className="text-muted-foreground">Our dApp is compatible with a wide range of Web3 wallets, allowing you to stake your assets securely and seamlessly.</p>
...
const IntegrationCard = ({ children, className, isCenter = false }: { children: React.ReactNode; className?: string; position?: 'left-top' | 'left-middle' | 'left-bottom' | 'right-top' | 'right-middle' | 'right-bottom'; isCenter?: boolean }) => {
    return (
        <div className={cn('relative flex size-12 rounded-full border bg-white shadow-sm shadow-black/5 dark:bg-white/5 dark:backdrop-blur-md', className)}>
